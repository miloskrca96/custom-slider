document.addEventListener("DOMContentLoaded", () => { 
    ...
    let sliderPaginationIndicator = document.querySelectorAll('.slider__pagination-indicator')[0];
    let messageBox = document.querySelectorAll('.slider__message-box')[0];
    let swipeInterval = (AUTOPLAY == 1) ? setInterval(changeSlideAutoplay, 1800) : null;
    ...
    if(ARROWS === 1) {
        arrowRight.addEventListener('click', () => {
            swipeAndResetInterval(numOfSlides, sliderPaginationIndicator, messageBox, slidesWrapper, swiperContainer, slideRight)
        })
    
        arrowLeft.addEventListener('click', () => {
            swipeAndResetInterval(numOfSlides, sliderPaginationIndicator, messageBox, slidesWrapper, swiperContainer, slideLeft)
        })
    ...
    swiperSlides.forEach(elem => {
        ...
        ['mousedown', 'touchstart'].forEach(eventN => {
            elem.addEventListener(`${eventN}`, (event) => {
                ...
                offset = (eventN === 'mousedown') ? event.clientX : event.touches[0].clientX;
                currentIndex = parseInt(elem.dataset.slideIndex);
            });
        });
        ...
        ['mousemove', 'touchmove'].forEach(eventN => {
            elem.addEventListener(`${eventN}`, (event) => {
                if (event.cancelable) event.preventDefault();
                if (isDown) {
                    mousePosition = (eventN === 'mousemove') ? event.clientX : event.touches[0].clientX
                    distanceSwiped = offset - mousePosition
                    const finalNum = parseInt(elem.dataset.slideIndex) * swiperContainer.offsetWidth + distanceSwiped
                    slidesWrapper.style.setProperty('transform', `translate3d(${-finalNum}px, 0, 0)`);
                }
            });
        });
    });
    ...
    for(let i=0; i < numOfSlides; i++) {
        // Classes
        const classes = (i == 0) ? ['pagination__bullet', 'active-bullet'] : ['pagination__bullet'];
        ...
        elem.addEventListener('click', () => {
            const currentSlide = document.querySelectorAll(`[data-slide-index="${i}"]`)[0];
            
            if(AUTOPLAY == 1) clearInterval(swipeInterval)
            changeSlide(currentSlide, elem, numOfSlides, sliderPaginationIndicator, messageBox, slidesWrapper, swiperContainer)
    ...
    function changeSlideAutoplay() {
        ...
        slideRight(numOfSlides, sliderPaginationIndicator, messageBox, slidesWrapper, swiperContainer)
    }

    // FUNCTION TO SLIDE LEFT
    function slideLeft(numOfSlides, sliderPaginationIndicator, messageBox, slidesWrapper, swiperContainer) {
        // Getting the next active slide, in this case the previous one
        const previousSlide = document.querySelectorAll('.current-slide')[0].previousElementSibling;
        // Getting the next active bullet, in this case the previous one
        const previousBullet = document.querySelectorAll('.active-bullet')[0].previousElementSibling;
        
        // Function which change active slide, bullet, pagination indicator and text in text box
        changeSlide(previousSlide, previousBullet, numOfSlides, sliderPaginationIndicator, messageBox, slidesWrapper, swiperContainer);
    }

     // FUNCTION TO SLIDE RIGHT
    function slideRight(numOfSlides, sliderPaginationIndicator, messageBox, slidesWrapper, swiperContainer) {
        // Getting the next active slide, in this case the next one
        const nextSlide = document.querySelectorAll('.current-slide')[0].nextElementSibling;
        // Getting the next active bullet, in this case the next one
        const nextBullet = document.querySelectorAll('.active-bullet')[0].nextElementSibling;
        
        // Function which change active slide, bullet, pagination indicator and text in text box
        changeSlide(nextSlide, nextBullet, numOfSlides, sliderPaginationIndicator, messageBox, slidesWrapper, swiperContainer);
    }

    // When autoplay is enabled, it is necessary to reset the interval when the slides are changed mechanically
    function swipeAndResetInterval(numOfSlides, sliderPaginationIndicator, messageBox, slidesWrapper, swiperContainer, swipeFunc) {
        // Stop interval
        if(AUTOPLAY == 1) clearInterval(swipeInterval)
        // Call function which is send as parameter
        swipeFunc(numOfSlides, sliderPaginationIndicator, messageBox, slidesWrapper, swiperContainer)
        // Start interval
        if(AUTOPLAY == 1)  swipeInterval = setInterval(changeSlideAutoplay, 1800);
    }

    function changeSlide(slideToShow, bulletToActive, numOfSlides, sliderPaginationIndDiv, messageBoxDiv, sliderWrapperDiv, swiperContainerDiv) {
        if(slideToShow == null) {
            if(AUTOPLAY == 0) {
        ...
        if(slideToShow !== document.querySelectorAll('.current-slide')[0]) {
            ...
            setPaginationIndicator(numOfSlides, parseInt(slideToShow.dataset.slideIndex) + 1, sliderPaginationIndDiv); 
        ...
        sliderWrapperDiv.style.setProperty('transform', `translate3d(${-num}px, 0, 0)`);
